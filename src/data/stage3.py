...
class Stage3TrajectoryDataset(Dataset[Stage3Example]):
    """Dataset that pairs teacher trajectories with frozen Stage 2 hidden states."""

    def __init__(
        self,
        teacher_dump_root: str | Path,
        split_file: str | Path,
        hidden_cache_dir: str | Path,
        ...
        self.hidden_cache_dir = Path(hidden_cache_dir)
        self.norm_stats = norm_stats
        self.manifests = self.teacher_dataset.manifests
        self.hidden_cache_paths = tuple(
            _validate_hidden_cache(self.hidden_cache_dir, manifest) for manifest in self.manifests
        )
        self.hidden_dim = _infer_hidden_dim(self.hidden_cache_paths)
    ...
    def __getitem__(self, index: int) -> Stage3Example:
        ...
        hidden_states = _load_hidden_cache(self.hidden_cache_paths[index], manifest.clip_id)
        hidden_count = int(hidden_states.shape[0])
        return {
            "clip_id": manifest.clip_id,
            "teacher_trajectories": torch.as_tensor(trajectories, dtype=torch.float32),
            "student_hidden_states": torch.as_tensor(hidden_states, dtype=torch.float32),
...
def collate_stage3_examples(examples: list[Stage3Example]) -> dict[str, Any]:
    ...
    hidden_dims = {int(example["student_hidden_states"].shape[1]) for example in examples}
    ...
    for example in examples:
        ...
        hidden_states = example["student_hidden_states"]
    ...
    return {
        ...
        "student_hidden_states": torch.cat(repeated_hidden, dim=0),
...
def _infer_hidden_dim(cache_paths: tuple[Path, ...]) -> int:
    if not cache_paths:
